...
namespace ColorSpace {
	...
	double Cie2000Comparison::Compare(IColorSpace *a, IColorSpace *b) {
		...
		double h1 = (double)fmod(atan2(lab_a.b, a1p) + 2 * M_PI, 2 * M_PI);
		double h2 = (double)fmod(atan2(lab_b.b, a2p) + 2 * M_PI, 2 * M_PI);
		...
		double rt = (double) - sin(DegToRad(60 * exp(-SQR((RadToDeg(meanH) - 275) / 25)))) * rc;
